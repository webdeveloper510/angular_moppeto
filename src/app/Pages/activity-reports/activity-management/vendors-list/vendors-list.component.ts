...
export class VendorsListComponent implements OnInit {
  ...
  onVendorNameSuggestion(event: any){
    ...
    this.http.post('http://127.0.0.1:8000/vendorNameSuggestion',{vendor_name:event.target.value}).subscribe(
      (response: any)=>{
        console.log('vendor_name_searchdata',response)
        console.log('vendor_name_searchdata',response.data.length)
...
if(data.dateRange[1]){
  ...
  var selected_date = data.dateRange
    ...
    let vendor_formData = {
      ...
      this.http.post('http://127.0.0.1:8000/activityfilterView', vendor_formData).subscribe(
        (response: any) => {
  ...
  onSearchChange(event: any){
    ...
    this.http.post('http://127.0.0.1:8000/activitySearchView',{vendor_search:event.target.value}).subscribe(
      (response: any)=>{
