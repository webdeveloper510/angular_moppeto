...
export class SearchWordsComponent implements OnInit {
  ...
  public leftShowButton: boolean[] = [false, false, false, false];
  ...
  changeLeftShowButton(index): void {
    this.leftShowButton[index] = true;
    for (let i = 0; i < this.leftShowButton.length; i++) {
      if (i != index) {
        this.leftShowButton[i] = false;
      }
    }
  }
